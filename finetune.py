...
model_path = './checkpoints/supPix_supervised.h5'  # Saving location
log_path = './checkpoints/supPix_supervised.csv'
weights_paths = None
...
model = attention_unet_refined(input_shape=image_size, mask_channels=3, out_channels=4,
                               multiplier=4, freeze_segmentor=False, dropout_rate=0.0)
metrics = [uniclass_dice_coeff_0, uniclass_dice_coeff_1, uniclass_dice_coeff_2, uniclass_dice_coeff_3]
...
if weights_paths is not None:
    model.load_weights(weights_paths, by_name=True)
    print("loaded!")

train_generator = DataGenerator(images_folder, train_image_list, batch_size=batch_size, mode='seg',
                                target_classes=target_classes,filter_classes=filter_classes, image_size=image_size, augment=True)
